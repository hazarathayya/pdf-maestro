...
def get_dl_details(dl):
    for images ,labels in dl:
        print(images.shape)
        print(labels.shape)
        for i in images:
            print(f"images's shape: {i.shape}, type: {type(i)}")
            break
        for j in labels:
            print(f"label's shape: {j.shape}, size: {j.size()} type: {type(j)}")
            break
        break

get_dl_details(train_dl)
...
model = to_device(CNN(3, 24), device)
print(model)
